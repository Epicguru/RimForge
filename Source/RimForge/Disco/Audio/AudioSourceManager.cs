...
namespace RimForge.Disco.Audio
{
    public static class AudioSourceManager
    {
        ...
        private static float volumeFar = 0.15f;

        [TweakValue("_RimForge", 0f, 100f)]
        private static float cameraClosest = 28;
        [TweakValue("_RimForge", 0f, 100f)]
        ...
        [DebugAction("RimForge", actionType = DebugActionType.Action)]
        ...
        public static void Tick(bool removeAll)
        {
            if (removeAll)
            {
                ...
                {
                    if(src.Source != null)
                        UnityEngine.Object.Destroy(src.Source.gameObject);
                });
                activeSources.Clear();
                return;
            }
            ...
            for (int i = 0; i < activeSources.Count; i++)
            {
                ...
                if (remove)
                {
                    ...
                    if(src.Source != null)
                        UnityEngine.Object.Destroy(activeSources[i].Source.gameObject);
        ...
        private static bool Tick(int index)
        {
            ...
            float vol = volumeMulti * item.TargetVolume;
